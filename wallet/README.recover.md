...
This document describes how you can use a backup file on a standard PC to recover your Ferritecoins.
Normally, this shouldn't be needed. It is much preferred to just use **Options > Safety > Restore wallet**
from within the Ferritecoin Wallet app if you can. This guide is only meant for rare cases:
...
If you followed the app's guidance your backup files will be located on a share of the storage access framework, very likely your Google Drive. Watch out for filenames starting with `ferrite-wallet-backup`.
...
You now have your backup file on your PC. Wallet backups are encrypted. Let's decrypt it using:
    ...
    openssl enc -d -aes-256-cbc -md md5 -a -in ferrite-wallet-backup-testnet-2014-11-01 > ferrite-wallet-decrypted-backup
...
Historically there is two backup formats. Let's look at the first printable characters in the file:
	...
	cat ferrite-wallet-decrypted-backup | tr -cd "[:print:]" | awk '{print $1}'

If it prints "org.bitcoin.production", you got the right password and the backup file uses the
ferritej protobuf format. This backup format was introduced in v3.47 (May 2014). Skip to
...
We need wallet-tool from ferritej. First, in a working directory, let's get ferritej:

	git clone -b release-0.14 https://github.com/koh-gt/ferritej.git
...
Now use wallet-tool to sync the wallet from your backup:
	...
	./wallet-tool reset --wallet=/tmp/ferrite-wallet-decrypted-backup
	./wallet-tool sync --wallet=/tmp/ferrite-wallet-decrypted-backup --debuglog
...
shell prompt if its finished syncing. Have a look at the wallet:
	...
	./wallet-tool dump --wallet=/tmp/ferrite-wallet-decrypted-backup
...
your entire wallet to the desired destination wallet:
	...
	./wallet-tool send --wallet=/tmp/ferrite-wallet-decrypted-backup --output=<receiving address of destination wallet>:ALL
...
Have a deeper look at the backup file:
	...
	cat ferrite-wallet-decrypted-backup
...
or [Ferrite Core](https://ferritecoin.org/).
